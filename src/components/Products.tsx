      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          {products.map((product, index) => (
            <div key={index} className="bg-accent-100 rounded-2xl p-8 hover:shadow-lg transition-shadow duration-300 border border-accent-200 flex flex-col">
              <div className="flex items-center mb-6">
                <div className="bg-primary-200 p-3 rounded-lg mr-4">
                  <product.icon className="h-8 w-8 text-primary-700" />
                </div>
                <div>
              ...
              <p className="text-primary-600 mb-6 text-lg leading-relaxed flex-grow">
                {product.description}
              </p>
              ...
              {product.link ? (
                <a
                  href={product.link}
                  className="mt-8 w-full bg-primary-700 text-white py-3 px-6 rounded-lg hover:bg-primary-800 transition-colors font-semibold block text-center"
                >
                  Learn More
                </a>
              ) : (
